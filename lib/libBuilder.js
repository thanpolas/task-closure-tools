...
builder.createCommand = function createCommand ( options, fileObj ) {
  ...
  if (canCompile) {
    ...
    // check for expandable directives and intercept with grunt file expand

    for( var i = 0; i < expandDirectives.length; i++ ) {
      directive = expandDirectives[i];
      // first check if we have an override for this directive on the fileObj.
      if (fileObj.hasOwnProperty(directive)) {
        // now expand the value and overwrite the compileOpts value
        compileOpts[directive] = gruntMod.file.expand(fileObj[directive]);
      } else if (compileOpts.hasOwnProperty(directive)) {
        // now expand the value
        compileOpts[directive] = gruntMod.file.expand(compileOpts[directive]);
      }
    }

    for( directive in compileOpts ) {
